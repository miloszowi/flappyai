...
export class Bird implements Entity {
  ...
  private lastInputData: Array<number> = [0, 0, 0];
  private lastRawInputData: Array<{ raw: number; normalized: number }> = [];
  ...
  private showStatsTooltip(): void {
    ...
    const gapDistance = this.lastRawInputData[0] ? this.lastRawInputData[0].raw.toFixed(1) : '0';
    const normalizedGapDistance = this.lastRawInputData[0] ? this.lastRawInputData[0].normalized.toFixed(3) : '0';
    const distanceToClosestPipe = this.lastRawInputData[1] ? this.lastRawInputData[1].raw.toFixed(1) : '0';
    const normalizedDistanceToClosestPipe = this.lastRawInputData[1] ? this.lastRawInputData[1].normalized.toFixed(3) : '0';
    const velocity = this.lastRawInputData[2] ? this.lastRawInputData[2].raw.toFixed(2) : '0';
    const normalizedVelocity = this.lastRawInputData[2] ? this.lastRawInputData[2].normalized.toFixed(3) : '0';
    this.statsTooltip.innerHTML = `
            <div class="stats-content">
                ...
                <div>Gap distance: ${gapDistance} (norm: ${normalizedGapDistance})</div>
                <div>Pipe distance: ${distanceToClosestPipe} (norm: ${normalizedDistanceToClosestPipe})</div>
                <div>Velocity: ${velocity} (norm: ${normalizedVelocity})</div>
                <hr style="margin: 5px 0; border: none; border-top: 1px solid #00ff00;">
  ...
  private decidedToJump(): number {
    ...
    this.lastInputData = [...input];
    this.lastRawInputData = [
      { raw: distToGapCenter, normalized: normalizedDistToGap },
      { raw: horizontalDist, normalized: normalizedHorizontalDist },
      { raw: this.velocity, normalized: normalizedVelocity }
    ];
  ...
  private generateNetworkVisualization(): string {
    ...
    const inputValues = this.lastInputData;
    ...
    setTimeout(() => {
      this.drawNetworkDiagram(canvasId, inputLabels, inputValues, hiddenNodes, outputNodes);
    }, 0);
